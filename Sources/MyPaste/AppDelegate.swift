...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, UNUserNotificationCenterDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        panelController.viewModel.uploadToImageHostingHandler = { [weak self] item in
            Task { @MainActor [weak self] in
                await self?.uploadImageToHosting(item: item)
            }
        }
    ...
    private func uploadImageToHosting(item: ClipboardItem? = nil) async {
        // 优先上传指定条目或面板中当前选中的图片/图片文件
        let viewModel = panelController.viewModel
        let selectedData: Data?
        let selectedFileName: String?
        var selectedItemError: String?
        ...
        if let item {
            ...
            targetItem = viewModel.filtered[viewModel.selectedIndex]
        } else {
            targetItem = nil
        }
        ...
        if let targetItem {
            ...
            case .image:
                if let data = viewModel.store.imageData(for: targetItem), let name = targetItem.imageFile, !name.isEmpty {
                    selectedData = data
                    selectedFileName = name
                } else {
                    selectedData = nil
                    selectedFileName = nil
                    selectedItemError = NSLocalizedString("选中图片无法读取", comment: "Selected image cannot be read")
                }
            case .file:
                if let path = targetItem.text,
                   FileManager.default.fileExists(atPath: path),
                   let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                    selectedData = data
                    selectedFileName = targetItem.fileDisplayName
                } else {
                    selectedData = nil
                    selectedFileName = nil
                    selectedItemError = NSLocalizedString("选中文件不存在或无法读取", comment: "Selected file does not exist or cannot be read")
                }
            default:
                selectedData = nil
                selectedFileName = nil
        ...
        } else if let error = selectedItemError {
            sendNotification(title: NSLocalizedString("图床上传失败", comment: "Image upload failed notification title"), body: error)
            return
        } else if let file = ImageHostingService.readFileFromPasteboard() {
            data = file.data
            fileName = file.fileName
        } else if let image = ImageHostingService.readImageFromPasteboard() {
            data = image.data
            fileName = image.fileName
        } else {
            sendNotification(title: NSLocalizedString("图床上传失败", comment: "Image upload failed notification title"), body: NSLocalizedString("未选中图片/文件且剪贴板中没有可上传内容", comment: "No selected image/file or uploadable content in clipboard notification body"))
            return
        }
        ...
        if result.success, let url = result.url {
            let isImage = data.isImageData
            let output = settings.imageHostingGenerateMarkdown && isImage ? (result.markdownUrl ?? url) : url
